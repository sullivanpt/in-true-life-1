<template>
  <div>
    ...
    <div v-if="!loading&&!exists" class="form-group">
      <label for="verify-password">Verify Password</label>
      <input v-model="verifyPassword" type="password" id="verify-password" :disabled="pending">
    </div>
    <div v-if="!loading&&!exists&&password&&password===verifyPassword" class="form-group">
    ...
    <p v-else-if="error==='CREATE_INVALID'" class="text-danger">this user name might be offensive to some people, please choose another</p>
  ...
  methods: {
    ...
    lookupName: function lookupName () {
      ...
      this.$api.meStrategies(this.name)
        .then(r => r.ok ? r.json() : null) // not 200 or 404 log error
        .then(j => {
          this.exists = j && this.name === j.user.name
          this.passwordToken = this.exists && j.password && j.password.token
          this.loading = !this.name // loading stays true if name is blank
        })
        .catch(e => {
    ...
    create () {
      ...
      this.$store.dispatch('me/create', {
        ...
        .then(r => {
          this.pending = false
          this.error = r ? null : 'CREATE_INVALID' // TODO: 'CREATE_EXISTS' etc, based on http status
    ...
    login () {
      ...
      this.$store.dispatch('me/login', {
        ...
        .then(r => {
          this.pending = false
          this.error = r ? null : 'LOGIN_CREDENTIALS'
          // note: no redirect needed, we are not changing pages
        })
        .catch(e => {
          this.pending = false
